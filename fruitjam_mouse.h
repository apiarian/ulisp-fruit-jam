// fruitjam_mouse.h — Mouse cursor for Fruit Jam
//
// Mouse position/button state (written by core1 USB host, read by core0),
// cursor sprite with save-under rendering, and hook wiring for the
...
// ---- Graphics mode flag ----
// Defined here (before fruitjam_graphics.h) because mouse_update_cursor()
// needs it, and both files are included into the same translation unit.
// fruitjam_graphics.h uses it for mode switching; extensions use it for
...
// ---- Init: wire up hooks ----
// Called from fruitjam_graphics_init() to connect the mouse cursor
// hide/peek functions to the FruitJamDisplay hook system.
static void fruitjam_mouse_init() {
  fruitjam_pre_draw_hook = mouse_hide_for_draw;
  fruitjam_peek_pixel_hook = mouse_peek_pixel;
}

#endif // FRUITJAM_MOUSE_H
